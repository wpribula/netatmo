...
class Room(Item):
    def _process_data(self, data: dict):
        ...
        self.modules_ids = data['module_ids'] if 'module_ids' in data.keys(
        ) else None
        self.home_id = None
    ...
    def _get_room_measure_url_data(self, type: str, from_time: datetime.datetime) -> dict:
    ...
    #                         TIMESERIES
    # =======================================================================================
    def load_room_time_series_data(self, type: str, from_time: pd._libs.tslibs.timestamps.Timestamp):
        data_json = self._netatmo_api.api_request('getroommeasure', self._get_room_measure_url_data(type, from_time))
        data_df = self._parse_temperature_response_real_time(data_json)
        data_df['room_id'] = str(self.id)
        data_df['type'] = type
        return data_df
    ...
    def _parse_temperature_response(self, response):
        values = []
        value_time = datetime.datetime.fromtimestamp(
            response['body']['home']['beg_time'])
        step = datetime.timedelta(
            seconds=response['body']['home']['step_time'])
...
class Rooms(Items):
    ...
    _time_series_types = ['temperature', 'sp_temperature']
    ...
    @classmethod
    def get_time_series(cls, from_date: datetime.datetime):
        cls._add_time_series_data_all()
        cls._save_time_series_data()
        if from_date:
            return cls._time_series_df.loc[cls._time_series_df['datetime'] > from_date].copy()
        return cls._time_series_df.copy()
    ...
    @classmethod
    def _check_time_series_data(cls):
        if cls._time_series_df.empty:
    ...
    def _load_temperatures_from_file(cls):
        try:
            cls._time_series_df = pd.read_csv(cls._data_path)
        except FileNotFoundError:
            cls._time_series_df = pd.DataFrame(
                {'room_id': [], 'type': [], 'datetime': [], 'value': []})
        cls._time_series_df['datetime'] = pd.to_datetime(
            cls._time_series_df['datetime'])
        cls._time_series_df['room_id'] = cls._time_series_df['room_id'].astype(
            str)


    @classmethod
    def _add_new_time_series_data(cls, new_data_df: pd.DataFrame):
        cls._check_time_series_data()
        cls._time_series_df = pd.concat([cls._time_series_df, new_data_df])


    @classmethod
    def _add_time_series_data_all(cls):
        for room_id in cls.items:
            cls._add_time_series_data_for_room_id(room_id)


    @classmethod
    def _add_time_series_data_for_room_id(cls, room_id):
        for type in cls._time_series_types:
            new_data_df = cls._load_new_time_series_data_for_type(room_id, type)
            cls._add_new_time_series_data(new_data_df)


    @classmethod
    def _load_new_time_series_data_for_type(cls, room_id, type):
        from_date = cls._get_latest_value_date(room_id, type)
        if (datetime.datetime.now() - from_date) > datetime.timedelta(minutes=30):
            return cls.items[room_id].load_room_time_series_data(type, from_date)


    @classmethod
    def _get_latest_value_date(cls, room_id, type_of_measurement) -> pd._libs.tslibs.timestamps.Timestamp:
        cls._check_time_series_data()
        filt = (cls._time_series_df['room_id'] == str(room_id)) & (
            cls._time_series_df['type'] == type_of_measurement)
        timestamp = cls._time_series_df.loc[filt, 'datetime'].max()
    ...
    @classmethod
    def _get_average_for_days(cls, data_df: pd.DataFrame) -> pd.DataFrame:
        resampled_df = data_df.loc[data_df['type'] ==
                                   'temperature', ['value', 'datetime']].copy()
        resampled_df.set_index('datetime', inplace=True)
        new_index = pd.date_range(
            resampled_df.index.min(), resampled_df.index.max(), freq='30min')
        resampled_df = resampled_df.reindex(resampled_df.index.union(
            new_index)).interpolate('index').reindex(new_index)
    ...
    def _get_day_by_day_plot(cls, data_df: pd.DataFrame, from_date: datetime.datetime, room_id: str):
        ...
        for _, day_data_df in data_df.groupby('days'):
            ...
            axs[ax_idx].set_ylim(
                [day_data_df['value'].min() - 1, day_data_df['value'].max() + 1])
            axs[ax_idx].set_yticks(np.arange(
